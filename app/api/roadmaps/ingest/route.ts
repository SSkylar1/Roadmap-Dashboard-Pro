...
import { insertStandaloneRoadmap } from "@/lib/standalone/roadmaps-store";
...
export async function POST(req: NextRequest) {
  try {
    ...
    let parsed: unknown;
    try {
      parsed = format === "yaml" ? loadYaml(source) : JSON.parse(source);
    } catch (e:any) {
    ...
    if (STANDALONE_MODE) {
      const title = normalized.title?.trim() || "Untitled Roadmap";
      const record = insertStandaloneRoadmap({
      ...
      return NextResponse.json({ ok: true, roadmap: record });
    }

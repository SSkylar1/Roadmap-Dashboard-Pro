...
type Check = { type: "files_exist" | "http_ok" | "sql_exists"; globs?: string[]; url?: string; must_match?: string[]; query?: string };
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (rmRaw === null) return NextResponse.json({ error:"docs/roadmap.yml missing" }, { status:404 });
    ...
    for (const w of rm.weeks ?? []) {
      ...
      for (const it of w.items ?? []) {
        let passed = true; const results:any[] = [];
    ...
    await putFile(owner, repo, "docs/roadmap-status.json", pretty, branch, "chore(roadmap): update status [skip ci]");
    ...
    await putFile(owner, repo, "docs/project-plan.md", plan, branch, "chore(roadmap): update plan [skip ci]");

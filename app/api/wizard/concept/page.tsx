  ...
  return (
    <div className="space-y-3 max-w-3xl">
      ...
      <div className="space-x-2">
        ...
        <label><input type="radio" checked={format==="json"} onChange={()=>setFormat("json")} /> JSON</label>
      </div>
      <textarea
        className="w-full h-80 border p-2 font-mono"
        placeholder={format==="yaml" ? "title: ...\nphases:\n  - name: ..." : '{ "title": "...", "phases": [...] }'}
        value={source}
        onChange={(e)=>setSource(e.target.value)}
      />
      <button onClick={handleSave} className="border px-3 py-2">Validate & Save</button>
    </div>
  );
}
